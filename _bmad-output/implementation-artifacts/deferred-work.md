...
- source_spec: `_bmad-output/implementation-artifacts/spec-rede-firestore-rules.md`
  summary: "[RESOLVIDO] O achado \"`firestore.rules` continua sem nenhuma rede automatizada\" (registrado acima, vindo da revisão adversarial de 25/09/2026) foi fechado: existe `testes/teste-regras.js`, suíte versionada que sobe o Firestore Emulator, carrega o `firestore.rules` do repositório e confere 160 verificações nos DOIS sentidos (quem pode e quem não pode) -- 153 garantias e 7 gaps aceitos, contados SEPARADAMENTE para ninguém ler o verde como "tudo protegido". `firebase.json` ganhou bloco `emulators` e passou a ser versionado, junto de `.firebaserc` e `firestore.indexes.json` (os três estavam untracked desde a auditoria inicial). O CI instala Java 21, guarda o .jar do emulador em cache e roda a suíte. O `CLAUDE.md` deixou de exigir \"teste no Emulator\" sem mecanismo: agora aponta `npm run test:regras` e proíbe afrouxar regra para fazer teste passar."
  evidence: "160/160 verificações no Firestore Emulator local (projeto `demo-mappo-regras`, sem credencial nem rede), exit 0, veredito impresso -- e a suíte entra sozinha no `npm test` pela regra de nome `teste-*.js`. A rede foi provada por dois afrouxamentos deliberados feitos numa CÓPIA das regras via `MAPPO_RAIZ` (o `firestore.rules` versionado não foi tocado): (1) retirar `isAtivo(wsId)` da leitura de `data` -> exit 1 nomeando \"cliente anonimo tenta LER pub_vivo de workspace PENDENTE\" e a regra; (2) trocar `isMember(wsId)` por `request.auth != null` -> exit 1 nomeando \"membro do workspace B tenta LER workspaces/wsA/data/mappo_os\". Os gaps já aceitos pelo proprietário viraram casos marcados `[GAP ACEITO]` com o porquê no próprio teste (técnico escreve `mappo_os` e `pub_*`; gestor cria ponteiro de uid arbitrário; convite legado sem `expiraEm`) -- eles agora falham se alguém mudar a decisão sem ele saber. Esta entrega TESTA as regras, não as corrige: `git diff firestore.rules` vazio."

- source_spec: `_bmad-output/implementation-artifacts/spec-rede-firestore-rules.md`
  summary: "`firestore.rules` NÃO garante o uso único do convite de prestador, apesar do comentário da própria regra afirmar \"consumido na mesma transação\". A regra de `workspaces/{wsId}/members/{uid}` apenas LÊ `convites/{codigo}` e exige `usado == false`; ela não obriga que o convite seja marcado como usado na mesma escrita. Quem montar a chamada Firestore à mão (fora da UI) cria a membership e simplesmente não consome o código -- e o mesmo convite serve a quantas pessoas quiserem, até alguém marcá-lo. \"Uso único\" é hoje garantia do cliente (`index.html`), não da regra."
  evidence: "Provado no Emulator, não deduzido: `testes/teste-regras.js` grupo 6 tem o caso `[GAP ACEITO] SEGUNDA pessoa se auto-vincula com o MESMO convite ainda não consumido` -- PERMITIDO hoje. O caso seguinte (`TERCEIRA pessoa ... DEPOIS de ele ser marcado como usado`) é NEGADO, o que delimita a janela: ela fecha só quando o convite é efetivamente marcado. Achado da revisão adversarial de 26/09/2026, que notou que todos os casos de convite eram `setDoc`/`updateDoc` soltos e nenhum afirmava o invariante central da Story 11. Corrigir exige a regra exigir a escrita conjunta (o Firestore não expressa isso entre documentos sem batch validado) ou mover o consumo para fora do cliente -- redesenho, não patch. Está documentado como gap aceito na suíte para não mudar sozinho; a decisão de corrigir é do proprietário."

- source_spec: `_bmad-output/implementation-artifacts/spec-rede-firestore-rules.md`
  summary: "[RESOLVIDO] A suíte de regras podia ficar verde cobrindo muito menos do que afirmava. Quatro defesas foram acrescentadas depois da revisão adversarial de 26/09/2026: (1) PISO de cobertura -- `PISO_CASOS`/`PISO_GRUPOS` conferidos no fim, então apagar um `grupo(...)` reprova em vez de passar verde com fração da cobertura; (2) ESPELHO -- a suíte lê as listas de `isGestorOnlyDoc()` (10 docIds) e `ramoValido()` (8 chaves) do próprio `firestore.rules` e reprova se divergirem das dela, então acrescentar item na regra sem acrescentar caso é falha; (3) leitura permitida CONFERE o conteúdo, porque para um membro a regra permite ler documento inexistente e \"não lançou\" não provava nada; (4) COLETA de falhas em vez de abortar na primeira."
  evidence: "Cada defesa verificada afrouxando de propósito numa CÓPIA das regras via `MAPPO_RAIZ` (o `firestore.rules` versionado nunca foi tocado -- `git diff --quiet HEAD -- firestore.rules` limpo): remover `mappo_settings` de `isGestorOnlyDoc` reprova DUAS vezes (a divergência da lista e a escalada do técnico); remover `prototype` de `ramoValido` reprova três vezes (lista + as duas caixas); apagar o grupo 7 inteiro reprova com `COBERTURA ABAIXO DO PISO -- casos: 147 (piso 160), grupos: 10 (piso 11)`; e com DUAS regras afrouxadas ao mesmo tempo o relatório lista as 11 falhas resultantes, não só a primeira -- mostrando que soltar `isMember` da leitura de `data` derruba junto a ancoragem de `pub_.*` e a expiração do link, blast radius que a versão fail-fast escondia. A suíte passou de 109 para 160 verificações."
