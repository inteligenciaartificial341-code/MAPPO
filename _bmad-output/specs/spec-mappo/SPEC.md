...
- **CAP-19 — Minitutorial contextual por tela**
  - **intent:** Cada tela do app ganha seu próprio mini-tour, disparando sozinho na primeira vez que o usuário entra nela, explicando os sub-elementos daquela tela especificamente — não só o nome dela, que o tour de topo (CAP-18) já cobre. Reaproveita o mesmo motor de spotlight/card do CAP-18, parametrizado por view em vez de rodar só uma vez no login.
  - **success:** Um usuário que entra pela primeira vez numa tela com sub-elementos (Ordens, Configurações, Financeiro, VRF, Mapa) vê um mini-tour explicando cada um; não reabre sozinho numa próxima visita àquela mesma tela; profundidade proporcional ao que a tela de fato tem, sem passo artificial em tela simples.
...
- CAP-19: corrige um bug real do CAP-18 no mesmo pacote — no modo Desktop (padrão do gestor), a barra de navegação de cima fica escondida (`display:none`), navegação vira menu flutuante; o tour do CAP-18 mirava só a barra escondida, então 5 passos nunca apareciam no modo padrão. O motor do tour passa a tentar uma lista de seletores candidatos em ordem, abrindo o container certo (sidebar ou menu flutuante) pra revelar o alvo, e fechando de novo ao sair — mesmo princípio já usado pra sidebar, estendido ao menu flutuante.
- CAP-19: cada view ganha array próprio de passos e flag "já visto" própria em `localStorage`, chaveada por uid E por nome da view — nunca reabre sozinho depois da primeira visita àquela tela, independente da flag do tour de topo. Botão "?" da sidebar continua reabrindo só o tour de topo. Escopo por view é proporcional ao que ela de fato tem (levantado por investigação real do código): Ordens (6 filtros + Nova OS), Configurações (7 blocos), Financeiro (3), VRF (sub-navegação + 3 ações), Mapa (mapa + Equipe + Histórico), Splits/Tarefas/Clientes/Manutenções (1 passo cada — só o botão de ação principal). Lado do técnico é mais raso, escopo exato a critério de quem implementar.
...
- CAP-19 não dá botão de reabrir dedicado pros mini-tours de tela nesta rodada — só o tour de topo (CAP-18) tem botão "?"; reabrir um mini-tour de tela sozinho só acontece se a versão do conteúdo daquela tela mudar (mesmo padrão de versão já usado pro tour de topo).
- CAP-19 não cobre a tela de execução de uma OS específica (checklist/fotos) — ela já tem instrução própria embutida na tela, e não é uma view estável visitável por `nav()` do mesmo jeito que as demais.
...
- CAP-19: levantamento exato de quais views do técnico ganham mini-tour e quantos passos cada uma, e o texto final de cada passo (gestor e técnico) — a decidir na hora de construir a story.
