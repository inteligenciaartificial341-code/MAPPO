...
- **Ambientes separados (dev/homolog/prod)** — não modelado nesta espinha; deploy continua manual. Commit em `main` é o caminho normal do projeto, mas exige autorização explícita do proprietário por commit — política registrada em `AGENTS.md`/`CLAUDE.md`, não uma AD desta espinha. (A afirmação anterior, de que `CLAUDE.md` proibia commit direto em `main`, deixou de valer em 25/09/2026.)
- **CI** — deixou de ser "não modelado": desde 25/09/2026 há GitHub Actions rodando `npm test` (as suítes de `testes/`) a cada push em `main` e a cada PR. Ele **não** publica nem faz deploy, e por rodar depois do commit já estar em `main` não bloqueia publicação por si só — bloquearia com proteção de ramo exigindo o status check, que ainda não está ligada.
